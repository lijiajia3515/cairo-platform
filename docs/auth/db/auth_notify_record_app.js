db.getCollection("auth_notify_record_app").drop();
db.createCollection("auth_notify_record_app", {
    validator: {
        $jsonSchema: {
            title: "应用级通知消息记录",
            description: "应用级通知消息记录",
            properties: {
                _id: {
                    bsonType: "objectId",
                    title: "数据库标识",
...
db.getCollection("auth_notify_record_app").createIndex({
    appId: NumberInt("1"),
    endpointId: NumberInt("1"),
    userId: NumberInt("1")
}, {
    name: "ix_appId_endpointId_userId",
    background: true
});
db.getCollection("auth_notify_record_app").createIndex({
    msgId: NumberInt("1")
