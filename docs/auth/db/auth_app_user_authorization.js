db.getCollection("auth_app_user_authorization").drop();
db.createCollection("auth_app_user_authorization", {
    validator: {
        $jsonSchema: {
            title: "应用级用户登录会话",
            description: "应用级用户登录会话",
            properties: {
                _id: {
                    bsonType: "objectId",
                    title: "数据库标识",
...
db.getCollection("auth_app_user_authorization").createIndex({
    appId: NumberInt("1"),
    endpointId: NumberInt("1"),
    clientId: NumberInt("1")
}, {
    name: "ix_appId_endpointId_clientId"
});
