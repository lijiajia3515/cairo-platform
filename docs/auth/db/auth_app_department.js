db.getCollection("auth_app_department").drop();
db.createCollection("auth_app_department", {
    validator: {
        $jsonSchema: {
            title: "部门",
            description: "部门",
            properties: {
                ...
                departmentId: {
                    ...
                    title: "部门ID",
                    description: "必填（可为 null），字符串，所属部门的唯一标识"
                },
                departmentName: {
                    bsonType: [
                        "null",
...
db.getCollection("auth_app_department").createIndex({
    appId: NumberInt("1")
}, {
    name: "ix_appId",
    background: true
...
db.getCollection("auth_app_department").createIndex({
    ...
    name: "ix_appId_departmentId_unique",
    background: true,
    unique: true
});
db.getCollection("auth_app_department").createIndex({
    ...
    name: "ix_appId_parentId_departmentName_unique",
    background: true,
    unique: true
});
