db.getCollection("auth_tenant_app_user_tag").drop();
db.createCollection("auth_tenant_app_user_tag", {
    validator: {
        $jsonSchema: {
            title: "用户标签",
            description: "用户标签",
...
db.getCollection("auth_tenant_app_user_tag").createIndex({
    "Metadata.TenantId": NumberInt("1"),
    "Metadata.AppId": NumberInt("1"),
    tagId: NumberInt("-1")
}, {
    name: "ix_tenantId_appId_tagId_unique",
    background: true,
    unique: true
});
db.getCollection("auth_tenant_app_user_tag").createIndex({
