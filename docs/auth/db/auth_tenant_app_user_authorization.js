db.getCollection("auth_tenant_app_user_authorization").drop();
db.createCollection("auth_tenant_app_user_authorization", {
    validator: {
        $jsonSchema: {
            title: "企业应用级用户登录会话",
            description: "企业应用级用户登录会话",
            properties: {
                _id: {
                    bsonType: "objectId",
                    title: "数据库标识",
...
db.getCollection("auth_tenant_app_user_authorization").createIndex({
    tenantId: NumberInt("1"),
    appId: NumberInt("1"),
    endpointId: NumberInt("1"),
    clientId: NumberInt("1")
}, {
    name: "ix_tenantId_appId_endpointId_clientId"
});
