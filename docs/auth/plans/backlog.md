...
- ⬜ **标准端点业务失败非纯 RFC（残留）**：Cairo 认证链 BusinessException（密码错等）不经 token 失败处理器、直接走全局异常处理器，`/oauth2/token` 上返回业务信封形态；协议层失败已纯 RFC。彻底化需在各自定义 Provider 把 Cairo 异常包装成 `OAuth2AuthenticationException`（动面大，open 代理已双形态透传保真，FE 不受影响）
- ⬜ **重启后最终验证矩阵**：①标准端点错误=纯 RFC ②open 面密码错误透传 `Auth.PasswordBad`（中间构建为 ParamsBad 回归）③authorize 未登录=302 /login ④SEC-09 登出后旧 token 被拒
