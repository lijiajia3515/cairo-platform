...
import {
  getTenantEndpointPageList_api,
  createTenantEndpoint_api,
  modifyTenantEndpointStatus_api,
  deleteTenantEndpoint_api,
  getAppList_api,
  getTenantList_api,
...
const [configs, setConfigs] = useState({
  ...
  columns: [
    ...
    switchColumn({
      api: modifyTenantEndpointStatus_api,
      idKeys: ['tenantId', 'appId', 'endpointId'],
      label: '企业终端',
      perm: 'tenant_endpoint.modify_tenant_endpoint_status',
      refresh: () => getTenantEndpointPage(),
    }),
    {
    ...
    opColumn([
      { content: '删除', theme: 'danger', onClick: (row) => onDelete(row), visible: () => hasPermission('tenant_endpoint.delete_tenant_endpoint') },
    ], { width: 160 })
  ],
  loading: loading,
  pagination: {
...
// watch(() => search.value.tenantId, async () => {
//   if (search.value.tenantId) {
//     let params = {
...
// 企业终端分页
const getTenantEndpointPage = async () => {
  setLoading(true);
  try {
    let params = {
      page: page.value - 1,
    ...
    let res = await getTenantEndpointPageList_api(params);
    if (res.code == 'Success') {
...
const onConfirm = async () => {
  ...
  if (validate == true) {
    ...
    if (type.value == 'add') {
      let res = await createTenantEndpoint_api(params);
      if (res.code == 'Success') {
        onClose();
...
const onDelete = (row) => {
  const confirmDia = DialogPlugin({
    ...
    onConfirm: async ({ e }) => {
      ...
      let res = await deleteTenantEndpoint_api(params);
      if (res.code == 'Success') {
        confirmDia.hide();
...
<template>
  <div v-allow="'tenant_endpoint.read'" class="tenantEndpoint__wrapper">
    <FilterBar @search="onSearch" @reset="onReset">
      ...
      <template #actions>
        <t-button v-allow="'tenant_endpoint.create'" @click="onCreate">创建</t-button>
      </template>
    </FilterBar>
    <div class="empty"></div>
