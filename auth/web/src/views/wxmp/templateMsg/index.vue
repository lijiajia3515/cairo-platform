...
defineOptions({ name: 'manage-wxmp-template_msg' })
...
import {
  getAppList_api,
  getWxmpTemplateMsgPageList_api,
  createWxmpTemplateMsg_api,
  modifyWxmpTemplateMsgInfo_api,
  getWxmpTemplateMsgDetailInfo_api,
  deleteWxmpTemplateMsg_api,
  modifyWxmpTemplateMsgStatus_api,
  getWxmpProviderList_api
} from '@/api';
...
const [configs, setConfigs] = useState({
  data: list,
  columns: [
    // {colKey: 'appId', title: '应用ID', ellipsis: true},
    ...
    // {colKey: 'jumpUrl', title: '跳转地址', ellipsis: true},
    switchColumn({
      api: (params) => modifyWxmpTemplateMsgStatus_api(params, {'app-id': headerData.value.appId}),
      idKeys: ['bizId'],
      label: '模板消息',
      perm: 'wxmp_template_msg.modify_status',
      refresh: () => getSmsTemplatePage(),
    }),
    opColumn([
      {content: '编辑', onClick: (row) => onEdit(row), visible: () => hasPermission('wxmp_template_msg.modify')},
      {content: '删除', theme: 'danger', onClick: (row) => onDelete(row), visible: () => hasPermission('wxmp_template_msg.delete')},
    ])
  ],
  loading: loading,
  pagination: {
...
const getSmsTemplatePage = async () => {
  ...
  try {
    ...
    let res = await getWxmpTemplateMsgPageList_api(params, headers);
    if (res.code == 'Success') {
...
const [wxmpProviderList, setWxmpProviderList] = useState([])
const getWxmpProviderList = async () => {
  let res = await getWxmpProviderList_api({Enabled: true})
  if (res.code === "Success") {
    setWxmpProviderList(res.data || [])
  }
}
...
const rules = {
  ...
  wxmpProviderId: [
    {required: true, message: '公众号ID必填', type: 'error', trigger: ''},
  ],
  templateSign: [
...
const [form, setForm] = useState({
  ...
  args: [{argCode: null, argName: null, argType: null, templateArgCode: null, DefaultColor: null}], // 模板参数编码
})
const onCreate = () => {
  setVisible(true);
...
const onConfirm = async () => {
  ...
  if (validate == true) {
    ...
    try {
      ...
      if (type.value == 'add') {
        let res = await createWxmpTemplateMsg_api(params, headers);
        if (res.code == 'Success') {
        ...
        let res = await modifyWxmpTemplateMsgInfo_api(params, headers);
        if (res.code == 'Success') {
...
const onEdit = async (row) => {
  ...
  let res = await getWxmpTemplateMsgDetailInfo_api(params, headers);
  if (res.code == 'Success') {
    const data = res?.data;
    setForm({
      bizId: data?.bizId, // 业务ID
      wxmpProviderId: data?.wxmpProviderId,
  ...
  const confirmDia = DialogPlugin({
    ...
    onConfirm: async ({e}) => {
      ...
      try {
        ...
        let res = await deleteWxmpTemplateMsg_api(params, headers);
        if (res.code == 'Success') {
          confirmDia.hide();
...
<template>
  <div v-allow="'wxmp_template_msg.find'" class="templateMsg__wrapper">
    <FilterBar @search="onSearch" @reset="onReset">
      ...
      <template #actions>
        <t-button v-allow="'wxmp_template_msg.create'" @click="onCreate">添加</t-button>
      </template>
    </FilterBar>
    <div class="empty"></div>
  ...
  <Dialog @confirm="onConfirm" @close="onClose" width="65%" :visible="visible" top="20">
    ...
    <t-form :rules="rules" ref="formRef" :data="form">
      <t-row>
        <t-col :span="6">
          <t-form-item name="bizId" label="业务ID">
            <t-input v-model="form.bizId" :disabled="type == 'edit'"></t-input>
          </t-form-item>
        </t-col>
        <t-col :span="6">
          <t-form-item name="wxmpProviderId" label="公众号ID">
            <t-select clearable v-model="form.wxmpProviderId">
              <t-option :label="item.wxmpProviderName" :value="item.wxmpProviderId"
                        v-for="(item, index) in wxmpProviderList" :key="index"></t-option>
            </t-select>
          </t-form-item>
        </t-col>
      </t-row>
      ...
      <t-row>
        ...
        <t-col :span="6">
          <t-form-item name="jumpUrl" label="跳转链接">
            <t-input v-model="form.jumpUrl"></t-input>
          </t-form-item>
        </t-col>
      </t-row>
      <div class="empty"></div>
            ...
            <div classs="paramsBox" style="width: 100%;">
              <div style="display: flex;width: 100%;marginBottom:10px;" v-for="(item, index) in form.args" :key="index"
                ...
                <t-form-item style="width: calc(((100% - 60px)/4) - 10px);" labelAlign="top" label="参数编码:">
                  <t-input style="width: 100%;" v-model="form.args[index].argCode"
                           placeholder="请输入参数编码"></t-input>
                </t-form-item>
                <t-form-item style="width: calc(((100% - 60px)/4) - 10px);" labelAlign="top" label="参数名称:">
                  <t-input style="width: 100%;" v-model="form.args[index].argName"
                           placeholder="请输入参数名称"></t-input>
                </t-form-item>
                <t-form-item style="width: calc(((100% - 60px)/4) - 10px);" labelAlign="top" label="参数类型:">
                  <t-input style="width: 100%;" v-model="form.args[index].argType"
                           placeholder="请输入参数类型"></t-input>
                </t-form-item>
                ...
                <t-form-item style="width: calc(((100% - 60px)/5) - 10px);" labelAlign="top" label="默认颜色:">
                  <t-input style="width: 100%;" v-model="form.args[index].defaultColor"
                           placeholder="请输入默认颜色"></t-input>
                </t-form-item>
                <t-form-item labelAlign="top" label="&nbsp;">
