...
const onSubmit = async () => {
  ...
  if (validate == true) {
    let { appId, appVersion, endpointId, releaseVersion, latestVersion, title, remark, force, webUrl, type } = form.value;
    if (type.value == 'add') {
      let params = {
        appId, appVersion, endpointId, releaseVersion, latestVersion, title, remark, force, webUrl, type
      };
      ...
      let params = {
        appId, appVersion, endpointId, releaseVersion, title, remark, force, webUrl, type
      };
...
<template>
  ...
  <Dialog @close="onCloseWatch" @confirm="onCloseWatch" top="20" width="60%" :visible="visibleDetail">
    ...
    <t-row>
      ...
      <t-col :span="12">
        <t-space>
          <span>web地址: {{ detailData?.webUrl }}</span>
        </t-space>
      </t-col>
    </t-row>
  </Dialog>
  ...
  <Dialog @confirm="onSubmit" @close="onClose" top="20" :visible="visible">
    ...
    <t-form ref="formRef" :data="form" :rules="rules" label-width="120px">
      ...
      <t-row>
        <t-col :span="12">
          <t-form-item label="web地址">
            <t-textarea v-model="form.webUrl" placeholder="请输入地址" />
          </t-form-item>
        </t-col>
      </t-row>
      <div class="empty"></div>
