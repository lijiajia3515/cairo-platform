...
const onSubmit = async () => {
  ...
  if (validate == true) {
    let { appId, appVersion, endpointId, releaseVersion, latestVersion, title, remark, force, iosAppStoreUrl, type } = form.value;
    if (type.value == 'add') {
      let params = {
        appId, appVersion, endpointId, releaseVersion, latestVersion, title, remark, force, iosAppStoreUrl, type
      };
      ...
      let params = {
        appId, appVersion, endpointId, releaseVersion, title, remark, force, iosAppStoreUrl, type
      };
...
<template>
  ...
  <Dialog @close="onCloseWatch" @confirm="onCloseWatch" top="20" width="60%" :visible="visibleDetail">
    ...
    <t-row>
      ...
      <t-col :span="12">
        <t-space>
          <span>ios应用商店地址: {{ detailData?.iosAppStoreUrl }}</span>
        </t-space>
      </t-col>
    </t-row>
  </Dialog>
  ...
  <Dialog @confirm="onSubmit" @close="onClose" top="20" :visible="visible">
    ...
    <t-form ref="formRef" :data="form" :rules="rules" label-width="120px">
      ...
      <t-row>
        <t-col :span="12">
          <t-form-item label="ios应用商店地址">
            <t-textarea v-model="form.iosAppStoreUrl" placeholder="请输入地址" />
          </t-form-item>
        </t-col>
      </t-row>
      <div class="empty"></div>
