...
/**
 * [client/api] account connect service
 */
@Component
@Slf4j
public class AccountSnsClientApiService {
	...
	public AccountSnsClientApiService(@Qualifier("mongoTemplate") MongoTemplate mongoTemplate,
	...
	@NewSpan
	@BizLog(
		bizId = "sns:get_account_sns_map",
		scope = "read",
		params = {
			@BizLog.Param(key = "args", value = "#args"),
		}
	)
	public Map<String, SnsInfo> getAccountSnsMap(@Validated GetAccountSnsMapArgs args) {
		Criteria criteria = Criteria
			.where(AccountSnsMongodb.FIELD.SNS_PARTNER_ID).is(args.getSnsProviderId())
			.and(AccountSnsMongodb.FIELD.ACCOUNT_ID).in(args.getAccountIds());
		Query query = Query.query(criteria);
		List<AccountSnsMongodb> accountConnectMongodbList = readMongoTemplate.find(query, AccountSnsMongodb.class, MongodbConstants.Collection.ACCOUNT_SNS);
		return accountConnectMongodbList.stream().collect(Collectors.toMap(
			AccountSnsMongodb::getAccountId,
			x -> SnsInfo.builder()
				.providerOpenId(x.getSnsPartnerOpenId())
				.build())
		);
	}
	...
	@NewSpan
	@BizLog(
		bizId = "account_sns:get_account_sns_list",
		scope = "read",
		params = {
	...
	public List<AccountSns> getAccountSnsList(String appId, String accountId, GetAccountSnsArgs args) {
		//查询三方厂商
	...
	@Lock4j(name = "bind_app_user_account_sns", keys = {"#accountId", "#snsToken"})
		...
		if (openAccountSnsMongodb != null && openAccountSnsMongodb.getAccountId() != null) {
			...
			if (!exists) {
				...
				if (accountMongodb == null || (accountMongodb.getPhoneNumber() == null && accountMongodb.getUsername() == null)) {
					throw new ConflictBusinessException("绑定失败（绑定第三方账号是当前唯一登录方式）");
				}
			}
			transactionTemplate.executeWithoutResult(status -> {
				try {
	...
	@Lock4j(name = "unbind_app_user_account_sns", keys = {"#accountId", "#snsPartnerId"})
	public void unbindAccountSns(@Valid @NotNull String accountId, @Valid @NotNull String snsPartnerId) {
		...
		Query openQuery = Query.query(openCriteria).limit(1);
		AccountSnsMongodb openAccountSnsMongodb = mongoTemplate.findOne(openQuery, AccountSnsMongodb.class, MongodbConstants.Collection.ACCOUNT_SNS);
		if (openAccountSnsMongodb == null) {
			throw new ConflictBusinessException("解绑失败（当前绑定信息不存在）");
		}

		if (openAccountSnsMongodb.getAccountId() == null) {
			transactionTemplate.executeWithoutResult(status -> {
				try {
					//解绑
					Update update = new Update();
		...
		if (openAccountSnsMongodb.getAccountId() != null) {
			//账号是否存在多种第三方登录方式
			Criteria accountSnsCriteria = Criteria.where(AccountSnsMongodb.FIELD.ACCOUNT_ID).is(openAccountSnsMongodb.getAccountId())
				.and(AccountSnsMongodb.FIELD.SNS_PARTNER_OPEN_ID).ne(openAccountSnsMongodb.getSnsPartnerOpenId());
			Query accountSnsQuery = Query.query(accountSnsCriteria);
			boolean exists = mongoTemplate.exists(accountSnsQuery, AccountSnsMongodb.class, MongodbConstants.Collection.ACCOUNT_SNS);
			if (!exists) {
				//账号
