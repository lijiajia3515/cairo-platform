...
/**
 * 账号验证码模式 authentication provider
 */
@Slf4j
public final class OAuthAccountVerifyCodeAuthenticationProvider implements AuthenticationProvider {
     ...
     * @since 0.2.3
     */
    public OAuthAccountVerifyCodeAuthenticationProvider(AuthenticationManager authenticationManager, AccountAuthorizationService authorizationService, OAuth2TokenGenerator<? extends OAuth2Token> tokenGenerator) {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        OAuthAccountVerifyCodeAuthenticationToken token = (OAuthAccountVerifyCodeAuthenticationToken) authentication;
        ...
        CairoAccountVerifyCodeAuthenticationToken cairoAccountVerifyCodeAuthenticationToken = new CairoAccountVerifyCodeAuthenticationToken(token.getPhoneNumber(), token.getVerifyCode());

        Authentication accountAuthentication = authenticationManager.authenticate(cairoAccountVerifyCodeAuthenticationToken);
        ...
        OAuth2AccessToken accountAccessToken = new OAuthAccountAccessToken(
        ...
        if (authorizedScopes.contains(OidcScopes.OPENID)) {
            tokenContext = tokenContextBuilder.tokenType(CairoOAuthTokenTypeConstants.ID_TOKEN_TOKEN_TYPE).authorization(authorizationBuilder.build()).build();
    ...
    public boolean supports(Class<?> authentication) {
        return OAuthAccountVerifyCodeAuthenticationToken.class.isAssignableFrom(authentication);
    }
