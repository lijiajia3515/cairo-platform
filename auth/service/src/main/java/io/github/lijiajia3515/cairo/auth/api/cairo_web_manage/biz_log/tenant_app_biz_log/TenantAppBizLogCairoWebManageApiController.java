...
/**
 * [cairo_web_manage/api] tenant app endpoint  biz log controller
 */
@Slf4j
@Validated
@RestController
@RequestMapping("/cairo_web_manage_api/tenant_app_biz_log")
@CairoSecurity(type = CairoSecurityType.CAIRO_WEB_MANAGE_USER)
@RequiredArgsConstructor
@BusinessResultBody
public class TenantAppBizLogCairoWebManageApiController {
	 ...
	 */
	@PostMapping("/get_tenant_app_biz_log_page_list")
	@PreAuthorize("hasAnyAuthority('app_admin', 'tenant_app_biz_log:all', 'tenant_app_biz_log:read')")
	@CairoContext
	public Page<TenantAppBizLog> getTenantAppBizLogPageList(@AuthenticationPrincipal CairoOAuthSubappUserPrincipal principal,
                                                                            @Validated @RequestBody(required = false) GetTenantAppBizLogArgs args) {
		...
		return tenantAppBizLogCairoWebManageApiService.getTenantAppBizLogPageList(tenantId, appId, endpointId, clientId, Optional.ofNullable(args).orElse(GetTenantAppBizLogArgs.builder().build()));
	}
}
