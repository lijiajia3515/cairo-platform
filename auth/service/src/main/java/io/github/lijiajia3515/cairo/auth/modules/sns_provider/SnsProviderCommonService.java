...
/**
 * common sns provider service
 */
@Slf4j
@Validated
@Component
public class SnsProviderCommonService {

	private final MongoTemplate readMongoTemplate;
	private final SnsProviderProperties snsProviderProperties;
	private final AppCommonService appCommonService;

	public SnsProviderCommonService(@Qualifier("readMongoTemplate") MongoTemplate readMongoTemplate,
	...
	private Criteria buildCriteria(GetSnsProviderArgs args) {
		...
		if (args.getAppId() != null && !args.getAppId().isEmpty()) {
			criteria.and(SnsProviderMongodb.FIELD.APP_ID).in(args.getAppId());
		}
	...
	List<SnsProvider> getSnsProviderList(List<SnsProviderMongodb> ms) {
		...
		Map<String, ProviderTypeProperties> providerTypeMap = snsProviderProperties.getProviderTypes().stream()
			.collect(Collectors.toMap(ProviderTypeProperties::getId, x -> x, (x1, x2) -> x1));

		Map<String, ProviderPartnerProperties> providerPartnerMap = snsProviderProperties.getProviderPartners().stream()
			.collect(Collectors.toMap(ProviderPartnerProperties::getId, x -> x, (x1, x2) -> x1));

		return ms.stream().map(x -> SnsProviderConverter.convertSnsProvider(x,appMap,providerTypeMap, providerPartnerMap)).collect(Collectors.toList());
	}


	public void checkSnsProviderId(MongoTemplate mongoTemplate, String snsProviderId) {
		Criteria criteria = Criteria.where(SnsProviderMongodb.FIELD.SNS_PROVIDER_ID).is(snsProviderId);
		Query query = Query.query(criteria);
		if (!mongoTemplate.exists(query, SnsProviderMongodb.class, MongodbConstants.Collection.SNS_PROVIDER)) {
			throw new ConflictBusinessException("snsProviderId错误");
		}
	}
