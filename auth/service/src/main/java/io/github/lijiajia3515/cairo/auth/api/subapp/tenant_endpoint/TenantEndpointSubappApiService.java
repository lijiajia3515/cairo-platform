...
/**
 * [subapp_user/api] tenant service
 */
@Slf4j
@Validated
@Component
public class TenantEndpointSubappApiService {
	...
	public TenantEndpointSubappApiService(RabbitTemplate rabbitTemplate,
	...
	List<MetadataTenantEndpoint> getTenantEndpointList(String appId, @Validated GetTenantEndpointArgs args) {
	...
	public Page<MetadataTenantEndpoint> getTenantEndpointPageList(String appId, @Validated GetTenantEndpointArgs args) {
		Criteria criteria = buildCriteria(appId,args);
	...
	public void createTenantEndpoint(String appId, @Validated CreateTenantEndpointArgs args) {
	 ...
	 */
	@NewSpan
	@Lock4j(name = "modify_tenant_endpoint_info", keys = {"#args.tenantId", "#appId", "#args.endpointId"})
	@BizLog(
	...
	@SneakyThrows
	public void modifyTenantEndpointInfo(String appId, @Validated ModifyTenantEndpointInfoArgs args) {
	 ...
	 */
	@NewSpan
	@Lock4j(name = "modify_tenant_endpoint_status", keys = {"#args.tenantId", "#appId", "#args.endpointId"})
	@BizLog(
	...
	@SneakyThrows
	public void modifyTenantEndpointStatus(String appId, @Validated ModifyTenantEndpointStatusArgs args) {
		String currentAccountId = CairoSecurityContextHolder.getSubappAccountId();
		transactionTemplate.executeWithoutResult(status -> {
			try {
				Criteria criteria = Criteria
		...
		rabbitTemplate.convertAndSend(
			cairoRabbitmqTool.getExchange().getName(CairoAuthRabbitmqExchange.AUTH),
			cairoRabbitmqTool.getRouteKey().getTenantAppKey(CairoAuthRabbitmqRouteKey.MODIFIED_TENANT_ENDPOINT_STATUS, args.getTenantId(), appId),
	 ...
	 */
	@NewSpan
	@Lock4j(name = "delete_tenant_endpoint", keys = {"#args.tenantId", "#appId", "#args.endpointId"})
	@BizLog(
	...
	@SneakyThrows
	public void deleteTenantEndpoint(String appId, @Validated DeleteTenantEndpointArgs args) {
		String currentAccountId = CairoSecurityContextHolder.getSubappAccountId();
		transactionTemplate.executeWithoutResult(status -> {
			try {
				Criteria criteria = Criteria
		...
		rabbitTemplate.convertAndSend(
			cairoRabbitmqTool.getExchange().getName(CairoAuthRabbitmqExchange.AUTH),
			cairoRabbitmqTool.getRouteKey().getTenantAppKey(CairoAuthRabbitmqRouteKey.DELETED_TENANT_ENDPOINT, args.getTenantId(), appId),
	 ...
	 * @return criteria
	 */
	private Criteria buildCriteria(String appId,GetTenantEndpointArgs args) {
		...
		Optional.ofNullable(appId).filter(x -> !x.isBlank()).ifPresent(x -> criteria.and(TenantEndpointMongodb.FIELD.APP_ID).is(x));
		...
		Optional.ofNullable(args.getEnabled()).ifPresent(x -> criteria.and(TenantEndpointMongodb.FIELD.ENABLED).is(x));
