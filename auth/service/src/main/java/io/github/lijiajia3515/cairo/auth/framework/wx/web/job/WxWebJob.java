package io.github.lijiajia3515.cairo.auth.framework.wx.web.job;
...
@Slf4j
@Component
public class WxWebJob {
	private final CairoWxWebService cairoWxWebService;
	private final MongoTemplate readMongoTemplate;

	public WxWebJob(CairoWxWebService cairoWxWebService,
                    @Qualifier("readMongoTemplate") MongoTemplate readMongoTemplate) {
		this.cairoWxWebService = cairoWxWebService;
		this.readMongoTemplate = readMongoTemplate;
	}


	@XxlJob("wxWebConfigJob")
	public void wxWebConfigJob() {
		XxlJobHelper.log("job start");
		try {
			//查询所有微信公众号配置信息
			Criteria criteria = Criteria
				.where(SnsProviderMongodb.FIELD.SNS_PROVIDER_TYPE).is(SnsType.WX_WEB.getTypeValue())
			...
			providerMongodbs.forEach(provider -> {
				//移除配置
				cairoWxWebService.removeConfig(provider.getSnsProviderId());
				...
				cairoWxWebService.addConfig(provider.getSnsProviderId(), wxMpProperties);
			});
