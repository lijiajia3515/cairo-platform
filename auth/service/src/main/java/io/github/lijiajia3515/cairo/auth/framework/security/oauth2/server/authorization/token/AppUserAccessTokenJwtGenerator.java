...
/**
 * app_user access token jwt generator
 */
public final class AppUserAccessTokenJwtGenerator implements OAuth2TokenGenerator<Jwt> {
	 ...
	 */
	public AppUserAccessTokenJwtGenerator(JwtEncoder jwtEncoder) {
	...
	public Jwt generate(OAuth2TokenContext context) {
		...
		if (context.getTokenType() == null || !CairoOAuthTokenTypeConstants.APP_USER_ACCESS_TOKEN.equals(context.getTokenType())) {
			return null;
		}
		OAuth2TokenFormat tokenFormat = (OAuth2TokenFormat) context.getRegisteredClient().getTokenSettings().getSettings().getOrDefault(CairoSettingNames.Token.APP_USER_ACCESS_TOKEN_FORMAT, OAuth2TokenFormat.SELF_CONTAINED);
		if (!OAuth2TokenFormat.SELF_CONTAINED.equals(tokenFormat)) {
			return null;
		}

		Authentication authentication = context.getPrincipal();
		CairoAuthAppUser user;
		if (!(authentication.getPrincipal() instanceof CairoAuthAppUser)) {
			return null;
		}
		user = (CairoAuthAppUser) authentication.getPrincipal();
		...
		Instant expiresAt = issuedAt.plus((Duration) registeredClient.getTokenSettings().getSettings().getOrDefault(CairoSettingNames.Token.APP_USER_ACCESS_TOKEN_TIME_TO_LIVE, Duration.ofMinutes(30)));
		...
		//app user claim
		claimsBuilder
			.claim(CairoOAuthParameterNames.AUTH_TYPE, APP_USER.getValue())
