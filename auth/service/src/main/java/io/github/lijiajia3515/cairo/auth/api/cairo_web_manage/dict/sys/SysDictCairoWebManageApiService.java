...
/**
 * [cairo_web_manage/api] system dict service
 */
@Slf4j
@Validated
@Component
public class SysDictCairoWebManageApiService {
	...
	SysDictCairoWebManageApiService(@Qualifier("mongoTemplate") MongoTemplate mongoTemplate,
	...
	@NewSpan
	@BizLog(
		bizId = "sys_dict:create_sys_dict",
		scope = "write",
		params = {
	...
	public void createSysDict(@Valid @NotNull String appId, @Validated CreateSysDictArgs args) {
		//查询字典id是否存在
		if (mongoTemplate.exists(Query.query(Criteria.where(SysDictMongodb.FIELD.APP_ID).is(appId)
			.and(SysDictMongodb.FIELD.DICT_ID).is(args.getDictId())), SysDictMongodb.class, MongodbConstants.Collection.SYS_DICT)) {
		...
		SysDictMongodb sysDictMongodb = SysDictMongodb.builder()
			...
			.icon(args.getIcon())
			.isCreateItem(Optional.ofNullable(args.getIsCreateItem()).orElse(true))
		...
		transactionTemplate.executeWithoutResult(status -> {
			try {
				mongoTemplate.insert(sysDictMongodb, MongodbConstants.Collection.SYS_DICT);
			} catch (Exception e) {
				log.debug("字典创建失败", e);
				status.setRollbackOnly();
				throw new ConflictBusinessException("字典创建失败");
			}
		});
	}
	...
	public void modifySysDictInfo(@Valid @NotNull String appId, @Validated ModifySysDictArgs args) {
		DictType type = DictType.typeValueOf(args.getDictType()).orElseThrow(() -> new ParamsErrorBusinessException(String.format("参数: 类型：%s 错误", args.getDictType())));
		...
		if (args.getIcon() != null) {
			update.set(SysDictMongodb.FIELD.ICON, args.getIcon());
		}
	...
	public void putSysDictItem(@Valid @NotNull String appId, @Validated PutSysDictItemArgs args) {
	...
	public void modifySysDictItemInfo(@Valid @NotNull String appId, @Validated ModifySysDictItemInfoArgs args) {
		UpdateResult result = transactionTemplate.execute(status -> {
			try {
				...
				if (args.getIcon() != null) {
					update.set(SysDictItemMongodb.FIELD.ICON, args.getIcon());
				}
	 ...
	 */
	@NewSpan
	@Lock4j(name = "move_sys_dict_item", keys = {"#appId","#args.dictId","#args.moveItemId"})
	@BizLog(
	...
	public List<MetadataSysDictItem> getSysDictItemInfo(@Valid @NotNull String appId, @Validated GetSysDictItemInfoArgs args) {
		Criteria sdiCriteria = Criteria
			.where(SysDictItemMongodb.FIELD.APP_ID).is(appId)
			.and(SysDictItemMongodb.FIELD.DICT_ID).is(args.getDictId())
			.and(SysDictItemMongodb.FIELD.ITEM_ID).is(args.getItemId());
		...
		if (itemMongodb == null) return null;
		...
		Set<String> metadataAccountIds = CairoAccountTool.getAccountMetadataAccountIds(itemMongodbList.stream().map(SysDictItemMongodb::getMetadata).collect(Collectors.toList()));
	...
	public List<MetadataSysDict> getSysDictList(@Valid @NotNull String appId, @Validated GetSysDictArgs args) {
		...
		return getMetadataSysDictList(sysDictMongodbList);
	}
	...
	public Page<MetadataSysDict> getSysDictPageList(@NotNull String appId, @Validated GetSysDictArgs args) {
		...
		List<SysDictMongodb> sysDictMongodbList = readMongoTemplate.find(query, SysDictMongodb.class, MongodbConstants.Collection.SYS_DICT);
		List<MetadataSysDict> contents = getMetadataSysDictList(sysDictMongodbList);
	...
	public MetadataSysDict getSysDictInfo(@Valid @NotNull String appId, @NotNull String dictId) {
		Criteria sdc = Criteria
			.where(SysDictMongodb.FIELD.APP_ID).is(appId)
			.and(SysDictMongodb.FIELD.DICT_ID).is(dictId);
		...
		Set<String> metadataAccountIds = CairoAccountTool.getAccountMetadataAccountIds(Collections.singleton(dictMongodb.getMetadata()));

		Map<String, Account> accountMap = accountClientApiService.getAccountList(GetAccountListArgs.builder().accountIds(metadataAccountIds).build()).stream().collect(Collectors.toMap(Account::getAccountId, g->g));
		return SysDictConverter.convertMetadataSysDict(dictMongodb, accountMap);
	}
	...
	public MetadataSysDict getSysDictDetailInfo(@Valid @NotNull String appId, @NotNull String dictId,Boolean itemEnabled) {
		Criteria sdc = Criteria
			.where(SysDictMongodb.FIELD.APP_ID).is(appId)
			.and(SysDictMongodb.FIELD.DICT_ID).is(dictId);
		Query sdq = Query.query(sdc);
		...
		Map<String, Account> accountMap = accountClientApiService.getAccountList(GetAccountListArgs.builder().accountIds(metadataAccountIds).build()).stream().collect(Collectors.toMap(Account::getAccountId, g->g));
		MetadataSysDict dict = SysDictConverter.convertMetadataSysDict(dictMongodb, accountMap);
		List<MetadataSysDictItem> items = itemMongodbList.stream()
			.map(x -> SysDictConverter.convertMetadataSysDictItem(x, accountMap))
			.collect(Collectors.toList());

		List<MetadataSysDictItem> subItems = Tree2Converter.build(items, ROOT_ID);
	...
	public List<MetadataSysDictItem> getSysDictSubItemList(@Valid @NotNull String appId, @Validated GetSysDictSubItemArgs args) {
		Criteria sdc = Criteria
			.where(SysDictMongodb.FIELD.APP_ID).is(appId)
			.and(SysDictMongodb.FIELD.DICT_ID).is(args.getDictId());
		Query sdq = Query.query(sdc);
	...
	public List<MetadataSysDictItem> getSysDictSubItemTreeList(@Valid @NotNull String appId, @Validated GetSysDictSubItemArgs args) {
		String parentItemId = Optional.ofNullable(args.getParentItemId()).orElse(ROOT_ID);
		Criteria sdc = Criteria
			.where(SysDictMongodb.FIELD.APP_ID).is(appId)
			.and(SysDictMongodb.FIELD.DICT_ID).is(args.getDictId());
		...
		List<MetadataSysDictItem> subItems = itemMongodbList.stream()
			.map(x -> SysDictConverter.convertMetadataSysDictItem(x, accountMap))
			.collect(Collectors.toList());

		return Tree2Converter.build(subItems, parentItemId);
	}
	 ...
	 */
	@NewSpan
	@Lock4j(name = "copy_sys_dict_by_app_id", keys = {"#appId","#args.copyAppId"})
	@BizLog(
		bizId = "sys_dict:copy_sys_dict_by_app_id",
		scope = "write",
		params = {
			@BizLog.Param(key = "appId", value = "#appId"),
			@BizLog.Param(key = "args", value = "#args"),
	...
	public void copySysDictByAppId(String appId, CopySysDictByAppIdArgs args) {
		//查询要复制字典,字典项
		Criteria sdCopyCriteria = Criteria
			.where(SysDictMongodb.FIELD.APP_ID).is(args.getCopyAppId());
		Query sdQuery = Query.query(sdCopyCriteria);
		List<SysDictMongodb> sdCopyMongodbs = mongoTemplate.find(sdQuery, SysDictMongodb.class, MongodbConstants.Collection.SYS_DICT);
		//发送复制系统字典消息
		sdCopyMongodbs.forEach(sd -> {
			try {
				rabbitTemplate.convertAndSend(
					cairoRabbitmqTool.getExchange().getName(CairoAuthRabbitmqExchange.AUTH),
					cairoRabbitmqTool.getRouteKey().getAppKey(CairoAuthRabbitmqRouteKey.COPY_SYS_DICT, appId),
					objectMapper.writeValueAsString(CopySysDictMessage.builder()
						.eventAccountId(CairoSecurityContextHolder.getSubappAccountId())
						.eventTime(LocalDateTime.now())
						.copyAppId(args.getCopyAppId())
						.currentAppId(appId)
						.dictId(sd.getDictId())
						.build()
					),
	...
	private List<MetadataSysDict> getMetadataSysDictList(List<SysDictMongodb> mongodbList) {
