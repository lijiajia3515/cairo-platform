...
@Slf4j
@Component
public class DeleteAppBizLogByDeletedClientQueueHandler {
	private final MongoTemplate mongoTemplate;
	private final ObjectMapper objectMapper;

	public DeleteAppBizLogByDeletedClientQueueHandler(@Qualifier("mongoTemplate") MongoTemplate mongoTemplate, ObjectMapper objectMapper) {
		this.mongoTemplate = mongoTemplate;
		this.objectMapper = objectMapper;
	}

	@RabbitListener(
		queues = {"#{deleteAppBizLogByDeletedClientQueue.getName()}"}
	)
	public void queueHandle(@Headers Map<String, Object> headers, @Payload String payload, Message message, Channel channel) throws IOException {
		try {
			log.debug("[deleted_app_biz_log_deleted_client] message handler start");
			DeletedClientMessage deletedClientMessage = objectMapper.readValue(payload, DeletedClientMessage.class);
			log.info("[deleted_app_biz_log_deleted_client] ===> 已删除的客户端： AppId: {} ClientId: {} EventCairoUserId: {} EventTime: {} ",
			...
			Criteria criteria = Criteria
				.where(BizLogAppMongodb.FIELD.APP_ID).is(deletedClientMessage.getAppId())
				.and(BizLogAppMongodb.FIELD.CLIENT_ID).is(deletedClientMessage.getClientId());
			...
			for (long i = 0; i < each; i++) {
				try {
					...
					log.debug("用户级业务日志删除成功: AppId: {} ClientId: {} DeletedCount: {}", deletedClientMessage.getAppId(), deletedClientMessage.getClientId(), deletedAppBizLogMongodbList.size());
				} catch (Exception e) {
			...
			log.debug("[deleted_app_biz_log_deleted_client] message handler end");
		} catch (RuntimeException e) {
			log.info("[deleted_app_biz_log_deleted_client] message handler error", e);
