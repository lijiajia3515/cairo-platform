package io.github.lijiajia3515.cairo.auth.api.tenant_endpoint.login_log.tenant_app_user_login_log;
...
import io.github.lijiajia3515.cairo.auth.domain.api.tenant_endpoint.login_log.tenant_app_user_login_log.GetMyTenantAppUserLoginLogArgs;
import io.github.lijiajia3515.cairo.auth.domain.api.tenant_endpoint.login_log.tenant_app_user_login_log.MyTenantAppUserLoginLog;
...
/**
 * [tenant_endpoint/api] tenant app endpoint user login log service
 */
@Slf4j
@Validated
@Component
public class TenantAppUserLoginLogTenantEndpointApiService {
	private final MongoTemplate readMongoTemplate;

	private final EndpointCommonService endpointCommonService;
	private final ClientCommonService clientCommonService;

	public TenantAppUserLoginLogTenantEndpointApiService(@Qualifier("readMongoTemplate") MongoTemplate readMongoTemplate, EndpointCommonService endpointCommonService, ClientCommonService clientCommonService) {
	...
	public Page<MyTenantAppUserLoginLog> getMyTenantAppUserLoginLogPageList(@Valid @NotNull String tenantId, @Valid @NotNull String appId, @Valid @NotNull String userId, @Validated GetMyTenantAppUserLoginLogArgs args) {
		...
		List<MyTenantAppUserLoginLog> contents = getMyEndpointUserLoginLogList(mongodbList);
	...
	public List<MyTenantAppUserLoginLog> getMyEndpointUserLoginLogList(List<TenantAppUserLoginLogMongodb> mongodbList) {
		...
		return mongodbList.stream().map(x -> myEndpointUserLoginLog(x, endpointMap, clientMap)).collect(Collectors.toList());
	}

	public MyTenantAppUserLoginLog myEndpointUserLoginLog(TenantAppUserLoginLogMongodb source, Map<String, Endpoint> endpointMap, Map<String, BasicClient> clientMap) {
