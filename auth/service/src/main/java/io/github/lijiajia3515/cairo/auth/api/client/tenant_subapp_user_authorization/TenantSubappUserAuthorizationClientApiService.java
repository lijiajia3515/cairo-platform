...
import io.github.lijiajia3515.cairo.auth.framework.security.tenant_subapp_user.CairoAuthTenantSubappUser;
...
@Validated
@Component
public class TenantSubappUserAuthorizationClientApiService {
	private final MongoTemplate readMongoTemplate;
	private final CairoAuthTenantSubappUserService cairoAuthTenantSubappUserService;

	public TenantSubappUserAuthorizationClientApiService(@Qualifier("readMongoTemplate") MongoTemplate readMongoTemplate,
															 CairoAuthTenantSubappUserService cairoAuthTenantSubappUserService) {
	...
	@NewSpan
	@BizLog(
		bizId = "tenant_subapp_user_authorization:get_tenant_subapp_user_authorization",
		scope = "read",
		params = {
			@BizLog.Param(key = "args", value = "#args")
		}
	)
	public TenantSubappUserAuthorizationModel getTenantSubappUserAuthorization(@Validated GetTenantSubappUserAuthorizationArgs args) {
		TenantSubappUserAuthorizationModel.TenantSubappUserAuthorizationModelBuilder<?, ?> builder = TenantSubappUserAuthorizationModel.builder();
		...
		try {
			...
			CairoAuthTenantSubappUser user = cairoAuthTenantSubappUserService.loadTenantSubappUserModel(authorization.getTenantId(), authorization.getAppId(), authorization.getEndpointId(), args.getSubappId(), args.getSubappVersion(), authorization.getUserId());
