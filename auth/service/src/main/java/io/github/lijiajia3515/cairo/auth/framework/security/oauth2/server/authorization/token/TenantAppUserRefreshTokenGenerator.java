...
public final class TenantAppUserRefreshTokenGenerator implements OAuth2TokenGenerator<OAuth2RefreshToken> {
	private final StringKeyGenerator refreshTokenGenerator = new TokenKeyGenerator(Base64.getUrlEncoder().withoutPadding(), 32, "taeurt_");

	@Nullable
	@Override
	public OAuth2RefreshToken generate(OAuth2TokenContext context) {
		if (!CairoOAuthTokenTypeConstants.TENANT_APP_USER_REFRESH_TOKEN.equals(context.getTokenType())) {
			return null;
		}
		...
		Instant expiresAt = issuedAt.plus((Duration) context.getRegisteredClient().getTokenSettings().getSettings().getOrDefault(CairoSettingNames.Token.TENANT_APP_USER_REFRESH_TOKEN_TIME_TO_LIVE, Duration.ofDays(7)));
		return new OAuthTenantAppUserRefreshToken(
