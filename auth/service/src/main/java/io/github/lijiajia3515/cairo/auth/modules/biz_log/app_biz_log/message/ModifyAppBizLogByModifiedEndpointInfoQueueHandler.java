...
@Slf4j
@Component
public class ModifyAppBizLogByModifiedEndpointInfoQueueHandler {
	...
	public ModifyAppBizLogByModifiedEndpointInfoQueueHandler(ObjectMapper objectMapper,
	...
	@RabbitListener(
		queues = {"#{modifyAppBizLogByModifiedEndpointInfoQueue.getName()}"}
	)
	public void queueHandle(@Headers Map<String, Object> headers, @Payload String payload, Message message, Channel channel) throws IOException {
		try {
			log.debug("[modify_app_biz_log_by_modified_endpoint_info] message handler start");
			ModifiedEndpointInfoMessage modifiedEndpointInfoMessage = objectMapper.readValue(payload, ModifiedEndpointInfoMessage.class);
			log.info("[modify_app_biz_log_by_modified_endpoint_info] ===> 已修改的终端：Id: {} AppId: {} OldEndpointId: {} OldEndpointName: {} NewEndpointId: {} NewEndpointName: {} EventCairoUserId: {} EventTime: {} ",
			...
			if (modifiedEndpointInfoMessage.getOldEndpointId().equals(modifiedEndpointInfoMessage.getNewEndpointId())) {
				...
				log.debug("[modify_app_biz_log_by_modified_endpoint_info] message handler end");
				return;
			}

			Criteria criteria = Criteria
				.where(BizLogAppMongodb.FIELD.APP_ID).is(modifiedEndpointInfoMessage.getAppId())
				.and(BizLogAppMongodb.FIELD.ENDPOINT_ID).is(modifiedEndpointInfoMessage.getOldEndpointId());

			Query query = Query.query(criteria);

			Update update = new Update();
			update.set(BizLogAppMongodb.FIELD.ENDPOINT_ID, modifiedEndpointInfoMessage.getNewEndpointId());
			update.currentDate(BizLogAppMongodb.FIELD.METADATA.UPDATE_TIME);

			UpdateResult updateResult = mongoTemplate.updateMulti(query, update, BizLogAppMongodb.class, MongodbConstants.Collection.BIZ_LOG_APP);
			...
			log.info("[modify_app_biz_log_by_modified_endpoint_info] message handler error", e);
