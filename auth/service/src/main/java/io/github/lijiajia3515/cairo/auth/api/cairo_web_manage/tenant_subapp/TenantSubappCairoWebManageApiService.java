...
/**
 * [cairo_web_manage/api] tenant app subapp service
 */
@Slf4j
@Validated
@Component
public class TenantSubappCairoWebManageApiService {
	...
	public TenantSubappCairoWebManageApiService(@Qualifier("mongoTemplate") MongoTemplate mongoTemplate,
	...
	List<MetadataTenantSubapp> getTenantSubappList(@Validated GetTenantSubappArgs args) {
	...
	public Page<MetadataTenantSubapp> getTenantSubappPageList(@Validated GetTenantSubappArgs args) {
		Criteria criteria = buildCriteria(args);
	...
	public void createTenantSubapp(@Validated CreateTenantSubappArgs args) {
		...
		rabbitTemplate.convertAndSend(
			...
			cairoRabbitmqTool.getRouteKey().getTenantAppKey(CairoAuthRabbitmqRouteKey.CREATED_TENANT_SUBAPP, args.getTenantId(), args.getAppId()),
	 ...
	 */
	@NewSpan
	@Lock4j(name = "modify_tenant_subapp_status", keys = {"#args.tenantId", "#args.appId", "#args.endpointId", "#args.subappId"})
	@BizLog(
	...
	@SneakyThrows
	public void modifyTenantSubappStatus(@Validated ModifyTenantSubappStatusArgs args) {
		transactionTemplate.executeWithoutResult(status -> {
			try {
				Criteria criteria = Criteria
					.where(TenantSubappMongodb.FIELD.TENANT_ID).is(args.getTenantId())
					.and(TenantSubappMongodb.FIELD.APP_ID).is(args.getAppId())
		...
		rabbitTemplate.convertAndSend(
			cairoRabbitmqTool.getExchange().getName(CairoAuthRabbitmqExchange.AUTH),
			cairoRabbitmqTool.getRouteKey().getTenantAppKey(CairoAuthRabbitmqRouteKey.MODIFIED_TENANT_SUBAPP_STATUS, args.getTenantId(), args.getAppId()),
	 ...
	 */
	@NewSpan
	@Lock4j(name = "delete_tenant_subapp", keys = {"#args.tenantId", "#args.appId", "#args.endpointId", "#args.subappId"})
	@BizLog(
	...
	@SneakyThrows
	public void deleteTenantSubapp(@Validated DeleteTenantSubappArgs args) {
		...
		rabbitTemplate.convertAndSend(
			cairoRabbitmqTool.getExchange().getName(CairoAuthRabbitmqExchange.AUTH),
			cairoRabbitmqTool.getRouteKey().getTenantAppKey(CairoAuthRabbitmqRouteKey.DELETED_TENANT_SUBAPP, args.getTenantId(), args.getAppId()),
	 ...
	 * @return criteria
	 */
	private Criteria buildCriteria(GetTenantSubappArgs args) {
		...
		Optional.ofNullable(args.getAppId()).filter(x -> !x.isBlank()).ifPresent(x -> criteria.and(TenantSubappMongodb.FIELD.APP_ID).is(x));
