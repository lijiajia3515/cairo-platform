 ...
 * @since 0.2.3
 */
public final class AccountAccessTokenJwtGenerator implements OAuth2TokenGenerator<Jwt> {
	 ...
	 */
	public AccountAccessTokenJwtGenerator(JwtEncoder jwtEncoder) {
	...
	public Jwt generate(OAuth2TokenContext context) {
		...
		if (context.getTokenType() == null || !CairoOAuthTokenTypeConstants.ACCOUNT_ACCESS_TOKEN.equals(context.getTokenType())) {
			return null;
		}
		...
		account = (CairoAuthAccount) authentication.getPrincipal();
		...
		// account claim
		claimsBuilder
			.claim(CairoOAuthParameterNames.AUTH_TYPE, ACCOUNT.getValue()) // auth_type
			...
			.claim(CairoOAuthParameterNames.ACCOUNT_ID, account.getAccountId()) // account_id
			.claim(CairoOAuthParameterNames.LOGIN_TYPE, account.getLoginType().getValue()) // login_type
		;
		if (account.getSnsType() != null && !account.getSnsType().isBlank()) {
			claimsBuilder.claim(CairoOAuthParameterNames.SNS_TYPE, account.getSnsType()); // login_type
		}
