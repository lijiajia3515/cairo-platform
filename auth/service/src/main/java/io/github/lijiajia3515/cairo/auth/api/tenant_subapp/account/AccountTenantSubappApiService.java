...
/**
 * [tenant_subapp_user/api] account service
 */
@Slf4j
@Validated
@Component
public class AccountTenantSubappApiService {
	private final MongoTemplate mongoTemplate;
	private final MongoTemplate readMongoTemplate;

	public AccountTenantSubappApiService(
	...
	public List<Account> getAccountList(@Validated GetAccountPageListArgs args) {
		...
		return records.stream()
			.map(x -> Account.builder()
				...
				.enabled(x.isEnabled())
				.locked(x.isLocked())
				.logoffStatus(x.getLogoffStatus())
	...
	public Account searchAccountInfo(@Validated SearchAccountArgs args) {
		...
		if (type.equals(SearchAccountArgs.Type.ACCOUNT_ID)) {
			...
			criteria.and(AccountMongodb.FIELD.EMAIL).is(args.getUsername().trim());
		} else {
