...
public final class AccountRefreshTokenGenerator implements OAuth2TokenGenerator<OAuth2RefreshToken> {
    ...
    @Nullable
    @Override
    public OAuthAccountRefreshToken generate(OAuth2TokenContext context) {
        if (!CairoOAuthTokenTypeConstants.ACCOUNT_REFRESH_TOKEN.equals(context.getTokenType())) {
            return null;
        }
        ...
        CairoAuthAccount user = (CairoAuthAccount) authentication.getPrincipal();

        Instant issuedAt = Instant.now();
        Instant expiresAt = issuedAt.plus((Duration) context.getRegisteredClient().getTokenSettings().getSettings().getOrDefault(CairoSettingNames.Token.ACCOUNT_REFRESH_TOKEN_TIME_TO_LIVE, Duration.ofDays(7)));
        return new OAuthAccountRefreshToken( this.refreshTokenGenerator.generateKey(), issuedAt, expiresAt);
    }
