package io.github.lijiajia3515.cairo.auth.api.cairo_web_manage.tenant_app;
...
import io.github.lijiajia3515.cairo.auth.domain.api.cairo_web_manage.tenant_app.CreateTenantAppArgs;
import io.github.lijiajia3515.cairo.auth.domain.api.cairo_web_manage.tenant_app.DeleteTenantAppArgs;
import io.github.lijiajia3515.cairo.auth.domain.api.cairo_web_manage.tenant_app.GetTenantAppArgs;
import io.github.lijiajia3515.cairo.auth.domain.api.cairo_web_manage.tenant_app.ModifyTenantAppInfoArgs;
import io.github.lijiajia3515.cairo.auth.domain.api.cairo_web_manage.tenant_app.ModifyTenantAppStatusArgs;
...
/**
 * [cairo_web_manage/api] tenant app service
 */
@Slf4j
@Validated
@Component
public class TenantAppCairoWebManageApiService {
	...
	public TenantAppCairoWebManageApiService(RabbitTemplate rabbitTemplate,
	...
	List<MetadataTenantApp> getTenantAppList(@Validated GetTenantAppArgs args) {
	...
	public Page<MetadataTenantApp> getTenantAppPageList(@Validated GetTenantAppArgs args) {
		Criteria criteria = buildCriteria(args);
	...
	public void createTenantApp(@Validated CreateTenantAppArgs args) {
		...
		if (insertedTenantAppMongodb != null) {
			rabbitTemplate.convertAndSend(
				...
				cairoRabbitmqTool.getRouteKey().getTenantAppKey(CairoAuthRabbitmqRouteKey.CREATED_TENANT_APP, args.getTenantId(), args.getAppId()),
	 ...
	 */
	@NewSpan
	@Lock4j(name = "modify_tenant_app_info", keys = {"#args.tenantId", "#args.appId"})
	@BizLog(
	...
	@SneakyThrows
	public void modifyTenantAppInfo(@Validated ModifyTenantAppInfoArgs args) {
		...
		rabbitTemplate.convertAndSend(
			cairoRabbitmqTool.getExchange().getName(CairoAuthRabbitmqExchange.AUTH),
			cairoRabbitmqTool.getRouteKey().getTenantAppKey(CairoAuthRabbitmqRouteKey.MODIFIED_TENANT_APP_INFO, args.getTenantId(), args.getAppId()),
	 ...
	 */
	@NewSpan
	@Lock4j(name = "modify_tenant_app_status", keys = {"#args.tenantId", "#args.appId"})
	@BizLog(
	...
	@SneakyThrows
	public void modifyTenantAppStatus(@Validated ModifyTenantAppStatusArgs args) {
		...
		rabbitTemplate.convertAndSend(
			cairoRabbitmqTool.getExchange().getName(CairoAuthRabbitmqExchange.AUTH),
			cairoRabbitmqTool.getRouteKey().getTenantAppKey(CairoAuthRabbitmqRouteKey.MODIFIED_TENANT_APP_STATUS, args.getTenantId(), args.getAppId()),
	 ...
	 */
	@NewSpan
	@Lock4j(name = "delete_tenant_app", keys = {"#args.tenantId", "#args.appId"})
	@BizLog(
	...
	@SneakyThrows
	public void deleteTenantApp(@Validated DeleteTenantAppArgs args) {
		...
		rabbitTemplate.convertAndSend(
			cairoRabbitmqTool.getExchange().getName(CairoAuthRabbitmqExchange.AUTH),
			cairoRabbitmqTool.getRouteKey().getTenantAppKey(CairoAuthRabbitmqRouteKey.DELETED_TENANT_APP, args.getTenantId(), args.getAppId()),
	...
	private Criteria buildCriteria(GetTenantAppArgs args) {
		...
		Optional.ofNullable(args.getAppId()).filter(x -> !x.isBlank()).ifPresent(x -> criteria.and(TenantAppMongodb.FIELD.APP_ID).is(x));
