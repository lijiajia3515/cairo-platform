...
@Validated
@Component
public class AppUserAuthorizationAppUserApiService {
	...
	public AppUserAuthorizationAppUserApiService(@Qualifier("mongoTemplate") MongoTemplate mongoTemplate,
	...
	/**
	 * 删除该会话 access token 的 Redis 缓存：登出/下线只改 Mongo 状态，不删缓存会让
	 * 资源侧状态检查（findByToken 缓存路径）在缓存 TTL 内读到旧 OK 记录，吊销不生效
	 */
	private void evictAccessTokenCache(AppUserAuthorizationMongodb authorization) {
		try {
			redisTemplate.delete(String.format("%s:%s:%s:%s", CairoAuthRedisConstants.Keys.APP_USER_ACCESS_TOKEN,
				authorization.getAppId(), authorization.getEndpointId(), authorization.getAccessToken().getTokenValue()));
		} catch (Exception e) {
			log.warn("evict app user access token cache failed", e);
		}
	}
