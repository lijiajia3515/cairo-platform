package io.github.lijiajia3515.cairo.auth.api.endpoint.account_sns;
...
/**
 * [endpoint/api] account sns service
 */
@Slf4j
@Validated
@Component
public class AccountSnsEndpointApiService {
	...
	public AccountSnsEndpointApiService(@Qualifier("mongoTemplate") MongoTemplate mongoTemplate,
	...
	public void unbindAccountSns(@Valid @NotNull String accountId, @Valid @NotNull String snsPartnerId) {
		Criteria openCriteria = Criteria
			.where(AccountSnsMongodb.FIELD.ACCOUNT_ID).is(accountId)
			.and(AccountSnsMongodb.FIELD.SNS_PARTNER_ID).is(snsPartnerId);
		Query openQuery = Query.query(openCriteria).limit(1);
		AccountSnsMongodb openAccountSnsMongodb = mongoTemplate.findOne(openQuery, AccountSnsMongodb.class, MongodbConstants.Collection.ACCOUNT_SNS);
