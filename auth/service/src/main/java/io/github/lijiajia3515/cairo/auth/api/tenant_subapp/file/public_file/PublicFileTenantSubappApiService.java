...
/**
 * [tenant_subapp_user/api]tenant app subapp public file service
 */
@Service
@Slf4j
public class PublicFileTenantSubappApiService {
	...
	public PublicFileTenantSubappApiService(MinioClient adminMinioClient, AsyncFileService asyncFileService, MinioProperties properties) {
	...
	@NewSpan
	@BizLog(
		bizId = "public_file:access_file_url",
		scope = "read",
		params = {
	...
	@NewSpan
	@BizLog(
		bizId = "public_file:upload_files",
		scope = "write",
		params = {
	...
	public List<List<String>> uploadFiles(String prefix, List<MultipartFile> files) {
		return files.parallelStream()
			.map(file -> {
				...
				try (InputStream in = file.getInputStream()) {
					String realPath = prefix.concat("/").concat(filename);
					final PutObjectArgs.Builder argsBuilder = PutObjectArgs.builder();
					argsBuilder.stream(in, -1, 1024 * 1024 * 100);
					...
					ObjectWriteResponse objectWriteResponse = this.adminMinioClient.putObject(argsBuilder.build());
					String version = objectWriteResponse.versionId();
					String s3Url = encodeS3Url(PUBLIC_BUCKET_NAME, realPath, version);
					String httpUrl = encodeS3PublicUrl(s3Url, endpoint, true);
					return List.of(filename, s3Url, httpUrl);
				} catch (IllegalArgumentException | IOException | ServerException | InsufficientDataException | ErrorResponseException |
	...
	@NewSpan
	@BizLog(
		bizId = "public_file:get_upload_file_sign",
		scope = "write",
		params = {
			@BizLog.Param(key = "args", value = "#args"),
		}
	)
	public UploadSignArgs getUploadFileSign(@Validated UploadFileSignArgs params) {
		ZonedDateTime time = ZonedDateTime.now().plus(params.getTtl());
		PostPolicy policy = new PostPolicy(PUBLIC_BUCKET_NAME, time);
		String keyPrefix = Optional.ofNullable(params.getKeyPrefix()).orElse("unknown/");
	...
	@NewSpan
	@BizLog(
		bizId = "public_file:get_upload_file_sign_url",
		scope = "read",
		params = {
			@BizLog.Param(key = "paths", value = "#paths"),
		}
	)
	public List<List<String>> getUploadFileSignUrl(@Valid @NotNull List<String> paths) {
		return paths.stream()
			.map(path -> {
				try {
					String s3Url = encodeS3Url(PUBLIC_BUCKET_NAME, path);
