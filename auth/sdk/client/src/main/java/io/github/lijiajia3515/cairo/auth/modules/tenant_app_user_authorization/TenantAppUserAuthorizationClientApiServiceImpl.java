...
public class TenantAppUserAuthorizationClientApiServiceImpl implements TenantAppUserAuthorizationClientApiService {
	...
	@Override
	public TenantAppUserAuthorizationModel getCustomTenantAppUserAuthorization(GetCustomTenantAppUserAuthorizationArgs args) {
		try {
			ResponseEntity<BusinessResult<TenantAppUserAuthorizationModel>> authorization = tenantAppUserAuthorizationClientApiFeignClient.getCustomTenantAppUserAuthorization(cairoOAuthClientSdkService.getHeaderAuthorization(), args);
			return Optional.ofNullable(authorization.getBody()).map(BusinessResult::getData).orElse(null);
		} catch (Exception e) {
			log.error("getCustomTenantAppUserAuthorization error", e);
			throw e;
		}
	}
}
