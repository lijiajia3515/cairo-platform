 ...
 * 用户客户端工具类
 */
public class CairoTenantAppUserClientTool {

	protected final TenantAppUserClientApiFeignClient tenantAppUserClientApiFeignClient;
	private final CairoOAuthClientSdkService cairoOAuthClientSdkService;

	public CairoTenantAppUserClientTool(TenantAppUserClientApiFeignClient cairoAuthRequestFeignClient, CairoOAuthClientSdkService cairoOAuthClientSdkService) {
		this.tenantAppUserClientApiFeignClient = cairoAuthRequestFeignClient;
	...
	public Map<String, TenantAppUser> getBasicUserByUserIds(String tenantId, String appId, Collection<String> userIds) {
		return Optional.ofNullable(userIds)
			.filter(x -> !x.isEmpty())
			.map(x -> {
				ResponseEntity<BusinessResult<List<TenantAppUser>>> userListResp = tenantAppUserClientApiFeignClient.getTenantAppUserList(
