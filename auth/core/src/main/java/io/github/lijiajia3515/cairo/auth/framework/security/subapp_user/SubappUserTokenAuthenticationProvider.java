...
public final class SubappUserTokenAuthenticationProvider implements AuthenticationProvider {

	private final Log logger = LogFactory.getLog(getClass());


	private SubappUserAuthenticationTokenConverter authenticationConverter = null;
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		if (!(authentication instanceof SubappUserAuthenticationTokenRequest user)) {
			return null;
		}
		Authentication result = this.authenticationConverter.convert(user);
		if (result == null) {
			return null;
		}
		...
		this.logger.debug("Authenticated app subapp token");
		return result;
	}

	@Override
	public boolean supports(Class<?> authentication) {
		return SubappUserAuthenticationTokenRequest.class.isAssignableFrom(authentication);
	}
...
//	}
	 ...
	 * @since 5.8
	 */
	public void setAuthenticationConverter(SubappUserAuthenticationTokenConverter authenticationConverter) {
