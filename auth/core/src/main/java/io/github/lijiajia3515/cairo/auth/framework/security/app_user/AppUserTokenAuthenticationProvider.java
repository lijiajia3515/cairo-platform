...
public final class AppUserTokenAuthenticationProvider implements AuthenticationProvider {

	private final Log logger = LogFactory.getLog(getClass());


	private AppUserAuthenticationTokenConverter authenticationConverter = null;
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		if (!(authentication instanceof AppUserAuthenticationTokenRequest tokenRequest)) {
			return null;
		}
		Authentication result = this.authenticationConverter.convert(tokenRequest);
		if (result == null) {
			return null;
		}
		if (AbstractAuthenticationToken.class.isAssignableFrom(result.getClass())) {
			...
			if (auth.getDetails() == null) {
				auth.setDetails(tokenRequest.getDetails());
			}
		}
		this.logger.debug("Authenticated app endpoint token");
		return result;
	}

	@Override
	public boolean supports(Class<?> authentication) {
		return AppUserAuthenticationTokenRequest.class.isAssignableFrom(authentication);
	}
...
//	}
	 ...
	 * @since 5.8
	 */
	public void setAuthenticationConverter(AppUserAuthenticationTokenConverter authenticationConverter) {
