...
public final class TenantAppUserAuthenticationTokenResolver {
	public static final String TENANT_ID = "tenantId";
	public static final String APP_ID = "appId";
	public static final String ENDPOINT_ID = "endpointId";
	public static final String TOKEN = "token";

	private static final String ACCESS_TOKEN_PARAMETER_NAME = "tenant_app_user_access_token";

	private static final Pattern AUTHORIZATION_PATTERN = Pattern.compile("^tenant_app_user (?<tenantId>[a-zA-Z0-9-._~+/]+=*)/(?<appId>[a-zA-Z0-9-._~+/]+=*)/(?<endpointId>[a-zA-Z0-9-._~+/]+=*)/(?<token>[a-zA-Z0-9-._~+/]+=*)$",
	...
	public TenantAppUserAuthenticationTokenRequest resolve(final HttpServletRequest request) {
		TenantAppUserAuthenticationTokenRequest headerToken = resolveFromAuthorizationHeader(request);
		final TenantAppUserAuthenticationTokenRequest parameterToken = isParameterTokenSupportedForRequest(request)
	...
	private TenantAppUserAuthenticationTokenRequest resolveFromAuthorizationHeader(HttpServletRequest request) {
		String authorization = request.getHeader(this.bearerTokenHeaderName);
		if (!StringUtils.startsWithIgnoreCase(authorization, "tenant_app_user")) {
			return null;
		}
		Matcher matcher = AUTHORIZATION_PATTERN.matcher(authorization);
		if (!matcher.matches()) {
			throw new TokenInvalidException("tenant_app_user 认证参数格式错误");
		}
		return new TenantAppUserAuthenticationTokenRequest(
	...
	private static TenantAppUserAuthenticationTokenRequest resolveFromRequestParameters(HttpServletRequest request) {
