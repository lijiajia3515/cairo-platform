 ...
 */
public final class AccountAuthenticationTokenResolver implements BearerTokenResolver {

	private static final String ACCESS_TOKEN_PARAMETER_NAME = "account_access_token";

	private static final Pattern authorizationPattern = Pattern.compile("^account (?<token>[a-zA-Z0-9-._~+/]+=*)$",
	...
	public String resolve(final HttpServletRequest request) {
		...
		if (authorizationHeaderToken != null) {
			if (parameterToken != null) {
				throw new TokenInvalidException("Found multiple bearer tokens in the request");
			}
	...
	private String resolveFromAuthorizationHeader(HttpServletRequest request) {
		...
		if (!StringUtils.startsWithIgnoreCase(authorization, "account")) {
			return null;
		}
		Matcher matcher = authorizationPattern.matcher(authorization);
		if (!matcher.matches()) {
			throw new TokenInvalidException("account 认证参数格式错误");
		}
