 ...
 */
public class CairoRequestRateLimiterGatewayFilterFactory
	extends AbstractGatewayFilterFactory<CairoRequestRateLimiterGatewayFilterFactory.Config> {
	...
	public CairoRequestRateLimiterGatewayFilterFactory(RateLimiter defaultRateLimiter, KeyResolver defaultKeyResolver, CairoWebfluxResponseHandler responseHandler, int order) {
		super(Config.class);
	...
	public GatewayFilter apply(Config config) {
		...
		GatewayFilter filter = (exchange, chain) -> resolver.resolve(exchange).defaultIfEmpty(EMPTY_KEY).flatMap(key -> {
			GatewayErrorBusinessResult<Object> responseEntity = GatewayErrorBusinessResult.<Object>builder()
			...
			if (EMPTY_KEY.equals(key)) {
				if (denyEmpty) {
					return responseHandler.handler(exchange, emptyKeyStatus.getHttpStatus(), responseEntity);
				}
